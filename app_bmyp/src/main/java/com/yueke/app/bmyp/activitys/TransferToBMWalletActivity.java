...
import com.yueke.app.bmyp.databinding.TransferToBmActivityBinding;
import com.yueke.app.bmyp.widgets.PayAmountEdittext;

@Route(path = ARouterPath.ActivityTransferToBMWallet)
public class TransferToBMWalletActivity extends BaseActivity {
    public static final String TransferlType = "TransferType";
    private TransferToBmActivityBinding mTransferToBmActivityBinding;
    ...
    private void initDataBinding() {
        mTransferToBmActivityBinding = DataBindingUtil.setContentView(this, R.layout.transfer_to_bm_activity);
    }

    private void initTopBar() {
        mTransferToBmActivityBinding.topBar.setTitle("转账到百盟钱包").getPaint().setFakeBoldText(true);
        mTransferToBmActivityBinding.topBar.addLeftBackImageButton().setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        //解决返回键不居中问题
        mTransferToBmActivityBinding.topBar.setGravity(Gravity.CENTER_VERTICAL);
    }
    ...
    private void initData() {
        mTransferToBmActivityBinding.setHandlers(this);
        mTransferToBmActivityBinding.setPageData(mTransferViewModel.getPageData());
        int type = getIntent().getIntExtra(TransferlType, 0);
    ...
    private void initViews() {
        mTransferToBmActivityBinding.mainLayout.setVisibility(View.GONE);
        mTransferToBmActivityBinding.etPayAmount.setButton(mTransferToBmActivityBinding.btTransfer);
        mTransferToBmActivityBinding.etPayAmount.setInputAccountListener(new PayAmountEdittext.InputAccountListener() {
            @Override
    ...
    public void userAccountEdittextChanged(CharSequence s, int start, int before, int count) {
    }
    ...
    public void doTransfer(View view) {
        ...
        if (TextUtils.isEmpty(mTransferViewModel.getPageData().getBMaccount().get())) {
            ToastUtils.info("请完善账号信息");
            return;
        }
        if (!RegexUtils.isMobileSimple(mTransferViewModel.getPageData().getBMaccount().get().replaceAll(" ", ""))) {
            ToastUtils.info("请输入手机号账户");
            return;
        }
        try {
        ...
        model.getPoundageScaleLiveData().observe(this, new Observer<WithDrawalBalanceAndScaleData>() {
            ...
            public void onChanged(@Nullable WithDrawalBalanceAndScaleData responseData) {
                if (responseData == null) {
                    mTransferToBmActivityBinding.emptyView.show(false, getResources().getString(R.string.emptyView_mode_desc_fail_desc), getResources().getString(R.string.emptyView_mode_desc_fail_desc), getResources().getString(R.string.emptyView_mode_desc_retry), retryAgainListener);
                    return;
                }
                if (responseData.isError()) {
                    mTransferToBmActivityBinding.emptyView.show(false, getResources().getString(R.string.emptyView_mode_desc_fail_title), null, getResources().getString(R.string.emptyView_mode_desc_retry), retryAgainListener);
                    resolveResponseErrorCode(responseData.getCode(), responseData.getMsg());
                    return;
                }

                mTransferToBmActivityBinding.emptyView.show(false);
                mTransferToBmActivityBinding.mainLayout.setVisibility(View.VISIBLE);
    ...
    private View.OnClickListener retryAgainListener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            mTransferToBmActivityBinding.mainLayout.setVisibility(View.GONE);
            mTransferToBmActivityBinding.emptyView.show(true);
            mTransferToBmActivityBinding.emptyView.setTitleText("正在加载");
            mTransferViewModel.getBalanceAndScale();
        }
    };
    ...
    public void showPayKeyBoard() {
        InputTools.HideKeyboard(mTransferToBmActivityBinding.layoutContent);
